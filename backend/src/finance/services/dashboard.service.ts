...
export async function getDashboardOverview(sessionId: string): Promise<DashboardOverviewRow | null> {
    ...
    const [overviewRes, expenseRes] = await Promise.all([
        ...
        getPool().query(expenseQuery, [session.start_date, session.end_date])
    ]);

    const overview = overviewRes.rows[0];
    const expense = expenseRes.rows[0];

    const income = Number(overview.total_collected || 0);
    const expenses = Number(expense.total_expenses || 0);
